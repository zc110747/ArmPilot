...
`robot-package/mearm-v1/tests/cases/*.json` 是"MeArm-V1 冻结时的行为快照"。
（目录由 manifest 的 `tests.cases` 声明，本脚本通过 `declared_path()` 取得 —— 不自己拼。）
...
sys.path.insert(0, str(PROJECT_ROOT / "core" / "python"))
...
#: 写进每个 JSON 的 `generator` 字段。**仓库相对路径**（人读得懂，且 `--check` 能顺带
#: 发现"生成器被搬走了但数据没重生成"）。搬包时它会变一次，之后应保持稳定。
GENERATOR = "robot-package/mearm-v1/tools/gen_mearm_v1_baseline.py"
...
def _out_dir() -> Path:
    """落盘目录 = 包在 **manifest 里声明的** `tests.cases`（唯一真值，不在这里拼）。

    Phase 2 之前这里硬编码 `tests/baseline/mearm-v1`；搬迁时那种写法必须同步改 N 处，
    而**漏改的那一处不会报错**（目录还在，只是内容已属于另一台机器人）。
    `must_exist=False`：生成器负责创建目录（否则首次生成会被自己的存在性检查挡住）。
    """
    try:
        return declared_path(MEARM_V1, "tests.cases", must_exist=False)
    except PackageError as exc:  # pragma: no cover - 只在包被搬坏时触发
        raise SystemExit(f"✗ 无法从 manifest 解析 {MEARM_V1} 的 tests.cases：{exc}") from exc
...
def check_documents(docs: dict[str, dict]) -> int:
    ...
    for name, doc in docs.items():
        path = out / name
        if not path.is_file():
            print(f"  ✗ 缺少 {path.relative_to(PROJECT_ROOT)}，请先不带 --check 跑一次")
            bad += 1
            continue
