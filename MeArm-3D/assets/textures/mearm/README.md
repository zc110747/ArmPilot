...
tiles/   tools/make_texture.py 生成的贴图（已按真实尺寸归一化）
```

## 怎么用

1. 读 `docs/texture-capture-guide.md` —— 拍摄清单（拍哪块板 / 大面朝向 / 四项硬性要求 / 自查清单）
2. 文件名权威清单：`python tools/make_texture.py --list`
3. 照片放进 `raw/<name>.jpg`
4. `python tools/make_texture.py --all`
...
透视校正、裁切、归一化全部在 `tools/make_texture.py` 里做，依据是照片里板的
