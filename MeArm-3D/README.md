...
**纹理已出 2 块**（`make_texture.py --corners`，四角取自信 D61 的"投影初值 + 梯度吸附"）：

| 纹理 | 尺寸 | 质量 |
|---|---|---|
| `forearm_link` | 18×68mm → 180×680px | ⚠️ 黑底 + 两枚螺栓 + **镂空透出的背景** |
| `upper_arm_link` | 22×74mm → 220×740px | ⚠️ 同上，另有一条**细线缆**从镂空透出 |

⚠️ **上一轮"线缆悬在板前面、软件无解"的判断过强，已更正（D61 四）**：4× 放大后看清，
橙色线缆在板的**背面**，是从**三角镂空里透出来**的。真正的问题是**四角把镂空区一起圈了进来**。
⇒ **不需要物理拨线**；但也说明剩下的问题**不是调四角能修的**。

⚠️ **根因是语义不匹配**：实板是**镂空桁架**，而模型的 `*_vis` 是**实心 box**
⇒ 照片里的"孔洞+透出的背景"烘到实心盒上会把背景色带进去（比不贴更假，见第 5 条）。
**必须做孔洞掩膜，或把镂空建成几何** —— 这是第 5 条待办，也是当前纹理不能入库的原因。
...
**下一步（按优先级）**：
- **A. 孔洞掩膜（推荐，纯软件）**：实板是镂空的、模型是实心 box ⇒ 在纹理阶段把孔洞
  **填成板面材质**（或反过来把镂空建成几何）。这一步做完，现有两块纹理就能用于渲染；
- **B. 补拍其余 7 块板**：需要额外姿态 / 机位；四角沿用 D61 的"投影初值 + 梯度吸附"；
- **C. 前端渲染接入**（按面材质数组 + 孔洞掩膜）验证渲染路径 —— 建议在 A 之后做，
  否则只会把背景色一起贴上去。

> ⚠️ 上一轮给的选项 A（"把线缆拨离板面"）**已作废**：线缆本就在板背面，拨它没有意义（D61 四）。
