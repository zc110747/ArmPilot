...
def _find_repo_root(start: Path) -> Path:
    """向上找同时含 `core/` 与 `robot-package/` 的那一层（深度无关 —— 见 gen_so_arm101_robot_yaml.py 同款说明）。"""
...
_CORE_PY = REPO / "core" / "python"
if str(_CORE_PY) not in sys.path:
    sys.path.insert(0, str(_CORE_PY))

from robopkg import declared_path  # noqa: E402

#: 路径只声明在包 manifest 里（Phase 2 起真值随包走）—— 这里不自己拼。
MODEL_XML = declared_path("so-arm101", "simulation.mjcf")
PHYSICS_YAML = declared_path("so-arm101", "model.physics")
