...
- **停止循环**：收到 **红外按钮 5**（`0xC53A05FA`）或**任何摇杆指令**（硬件摇杆移动 / 串口 `JOY`）。
...
- **新增动作序列键**（NEC 码格式 `地址<<24 | ~地址<<16 | 命令<<8 | ~命令`，地址取
  0xC1/0xC3/0xC5/0xC7/0xC9 避开原 8 键，互不冲突）：

  | 按钮 | NEC 32-bit 码 | 动作 |
  |------|---------------|------|
  | 1    | `0xC13E01FE`  | 启动动作集 1（抓取放置） |
  | 3    | `0xC33C03FC`  | 启动动作集 3（左右摇摆） |
  | 7    | `0xC73807F8`  | 启动动作集 7（俯仰） |
  | 9    | `0xC93609F6`  | 启动动作集 9（开合旋转） |
  | 5    | `0xC53A05FA`  | 停止当前循环 |

  真实遥控器按键码以实际遥控器为准；若不一致，按上述格式替换 `ir_ctrl.c` 中
  `IR_TAB` 的 `code` 即可（串口 `IR <hex>` 可注入任意 32-bit 码测试）。
