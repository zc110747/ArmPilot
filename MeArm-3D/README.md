...
2. **动关节改变不了分辨率 —— `px/mm` 只由物理距离决定。**
   ...
   ⇒ 相机上限 **1280×720**，当前标定 `s ≈ 3.8 px/mm`（74mm 大臂板 → 281px）。目标 ≥6 需物理靠近约 1.6×。
...
`--home` 保证实验从已知状态出发）；**相机标定 + 板件投影工具已落地**（`tools/project_plates.py`）。
**但自动投影精度不足**，三块纹理经目视复核判定不合格，**已删除不出库**。

**下一步（等你定，二选一）**：
- **A. 把机械臂挪近相机约 1.6×**（**唯一**能提高分辨率的手段）⇒ 再跑 `--search` 重标定，精度会明显改善；
- **B. 不动位置，改用人工标四角**：`python tools/make_texture.py --corners`（每块板一次，共 4 块）。
