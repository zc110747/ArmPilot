#!/usr/bin/env python3
"""由官方 SO-ARM101 模型生成 ArmPilot 的 `robot-package/so-arm101/model/robot.yaml`。
...
def _find_repo_root(start: Path) -> Path:
    ...
    刻意不用 `parents[N]`：本脚本从 `tools/` 搬进 `robot-package/so-arm101/tools/` 后，
    `parents[1]` 会指向 `robot-package/so-arm101`（真实存在但完全不对），
    错误会推迟到很远的地方才暴露。向上找标记则在任意深度都成立。
    """
...
#: 官方 MJCF 的路径 —— 从 `robot-package/so-arm101/manifest.yaml` 的
#: `simulation.mjcf` **声明**里取，本脚本不自己拼 `assets/models/...`。
#:
#: ★ Phase 2：这三行原先写的是
...
#:   `OUT_PATH` 在真值随包搬走后**指向了一个不再存在的文件** —— 而它的报错是
#:   "缺少 config\robots\so-arm101\robot.yaml"，看起来像"文件丢了"，
#:   而不是"有读者还在用旧路径"。这类漏改正是 `declared_path()` 要消灭的东西。
#:   URDF 由 MJCF 的**同名字干**推出（官方包里两份文件名始终同干）——
#:   改名会立刻在这里报错，不会静默读到别的文件。
try:
    MJCF_PATH = declared_path(SO_ARM101, "simulation.mjcf")
    OFFICIAL_DIR = MJCF_PATH.parent
    URDF_PATH = MJCF_PATH.with_suffix(".urdf")
    OUT_PATH = declared_path(SO_ARM101, "model.config", must_exist=False)
except PackageError as exc:  # pragma: no cover - 只在包被搬坏时触发
    raise SystemExit(f"✗ 无法从 manifest 解析 {SO_ARM101} 的声明路径：{exc}") from exc
