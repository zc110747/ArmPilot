...
import { SO_ARM101_ROBOT_ID } from '@robot/model/robotIds';
import type { JointState, Pose, Vec3 } from '@robot/model/Pose';
import { forwardKinematics } from '@robot/kinematics/fk';
import { ikFailure, type IKResult } from '@robot/kinematics/IKResult';
import type {
  InverseOptions,
 ...
 * **只认这一个名字**（与 `robot-package/mearm-v1/kinematics/engine.ts` 同一个契约）。
 *
 * 强制接收 `definition`：见 MeArm 侧同款说明。
 */
export function createKinematicsEngine(definition: RobotDefinition): KinematicsEngine {
  return new SoArm101Kinematics(definition);
}
