...
2. **动关节改变不了分辨率 —— `px/mm` 只由物理距离决定。**
   ...
   ⇒ 相机上限 **1280×720**。⚠️ **当前 `px/mm` 尚不可信**：旧值 `s = 3.8` 实为**搜索区间下界
   被截断**的结果（放宽区间后变成 2.58，两者互相矛盾），详见 D59 ①。**只能按"越近越大"定性地走。**
...
4. **目标板四角 = 相机标定 + 模型投影**（`tools/project_plates.py`）。
   正交模型足够：三块目标板的大面法向实测都是 `(0,1,0)` ⇒ 板面**平行于像平面** ⇒
   投影退化为「缩放 + 平移 + 面内旋转」（`u = ox + s·X`、`v = oy − s·Z`）。
   三判据**相乘**：框内暗色 × 空隙亮色 × 框边命中暗亮边界，其中**空隙项是防退化的关键** ——
   实测只优化"框内暗色"时，`s=2.5` 会把三块板挤成一团塞进左下底座暗区，拿到 97.4% 的
   **假性满分**（而 `oy=770` 已在画面外）。
   ⚠️ **但标定精度尚不足以直接出纹理**：实测框大致落在板上，边缘却会夹进橙色线缆/背景，
   3 块纹理经目视复核**判定不合格并已删除**。⇒ 必须**目视复核叠加图 + 纹理**，
   必要时改用 `make_texture.py --corners` 人工标四角。列为待办。
...
`--home` 保证实验从已知状态出发）；`project_plates.py` 已加**距离扫描模式**
（`--capture` 抓帧 + 一句话 `CONCLUSION` 报 `px/mm`／边缘过渡／是否出画 + **搜索边界检测**）。

**但"自动定位目标板"这条路在当前场景走不通**（D59）：
① 旧标定 `s = 3.80` 是**搜索下界被截断**的值，放宽区间后变成 2.58 —— 两次"测量"互相矛盾；
② 放宽后投影**依然对不上**（最优 score 仅 18.2，叠加图三个框都没贴板边）；
③ 实测**机械臂被画面顶边裁掉**（`y=0` 处 x[506,760] 连续暗带）。三块纹理已删除不出库。

**已换用「致动并观察」**：动一个关节、拍前后两帧差分 ⇒ 差分区域就是该关节及其下游零件。
实测 `shoulder/elbow/base` 三个动作各改变画面 **19.9% / 14.3% / 16.1%** 的像素
⇒ **物理臂确实在跟命令走**（排除了掉电/堵转/打滑）。该方法能做**归属判定与粗定位**，
但**给不出干净的单板四角**（肩座/平行杆/线缆连成一片），精确四角仍需人工标 + 目视复核。

**下一步（两件事都必须做，顺序不能颠倒）**：
- **① 重新取景 + 物理靠近**（**唯一**能提高分辨率的手段，且当前臂被顶边裁掉、目标板不完整）：
  让**三块目标板完整入画、四周留边**，并整体**靠近相机到目标 `px/mm`**（越近越大）；
- **② 人工标四角**：`python tools/make_texture.py --corners`（每块板一次），
  配合 motion-diff 的粗定位找候选区域。**自动投影不再作为出纹理的依据。**

照片/四角到位后再做**前端渲染接入**（按面材质数组 + 孔洞掩膜）—— 届时效果才可验证。
