 ...
 *                                     │
 *                                     ▼
 *                        robot-package/mearm-v1/tools/verify_pose.py（骨架拟合反解肩/肘绝对角）
 *                                     │
 *                                     ▼
 ...
 * 用法
 * ----
 *     node core/tools/verify_serial_e2e.mjs                 # 完整跑（会真的驱动机械臂）
 *     node core/tools/verify_serial_e2e.mjs --dry-run       # 只打印动作计划，不碰硬件
 *     node core/tools/verify_serial_e2e.mjs --no-camera     # 只验链路（不抓帧、不反解）
 *     node core/tools/verify_serial_e2e.mjs --reuse-backend # 复用已在 8090 的实例
 *     node core/tools/verify_serial_e2e.mjs --only 01,05    # 只跑指定步骤
 *
 * 前置
 * ----
