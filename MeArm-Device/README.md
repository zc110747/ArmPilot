...
- `STATS [CLEAR]`：打印**链路层丢弃计数** `OK STATS rx_drop=N tx_drop=N`；
  带 `CLEAR` 时先读后清零（便于"跑一轮压测再看这一轮增量"）。
  ★ 这两条计数是「gripper 偶发不执行」这类问题的**唯一直接判据**：
  链路真的丢字节，`rx_drop` 必然非 0；`rx_drop=0` 即可**否定**"链路丢字节"这一假设。
  见下方「UART 丢弃计数」一节。
- `HELP`：打印帮助
...
### UART 丢弃计数（`STATS` 命令，2026-09-15）

**为什么必须有**：链路层丢字节原本是**完全静默**的 —— RX 环形缓冲写满时直接丢弃，
没有任何外部信号。症状因此只表现为"偶发不执行"（如 `ERR 固件未应答 SET（>600ms）`），
既不可复现也无法归因。

**曾经的候选链条**（`uart_putc` 忙等 → 主循环停读 RX → 环满静默丢字节）经真机实测
**不成立**：`rx_drop=0 tx_drop=0`。见下方"实测结论"。

**两条计数**（`bsp/uart.c`，8 位饱和计数）：
| 计数 | 含义 | 触发点 |
|---|---|---|
| `rx_drop` | RX 环满而丢弃的**接收**字节数 | `ISR(USART_RX_vect)` |
| `tx_drop` | TX 环满而拒绝排队的**发送**字节数 | `uart_putc()` |

**`uart_putc()` 不再阻塞主循环**：TX 环由 128B 放大到 256B，环满时**放弃该字节并计数**，
而不是 `while (tx_full())` 无限期阻塞 —— 阻塞会让主循环停止读 RX，
反而可能丢掉**整条指令**（无任何痕迹）。宁可丢一个字符并且"记下来"。

**怎么用**：
```
STATS          -> OK STATS rx_drop=0 tx_drop=0
STATS CLEAR    -> 读完后清零（跑一轮压测只看这一轮增量）
```

**实测结论（2026-09-15，真机 COM18，120 帧 × 2 条 SET，33ms 节拍）：**
| 场景 | 发送 | 收到 | 含 S6 | ERR | 缺失 | rx_drop | tx_drop |
|---|---|---|---|---|---|---|---|
| gripper 扫动 120 帧 | 240 | 240 | 120 | 0 | **0** | **0** | **0** |
| gripper 全行程 120 帧 | 240 | 240 | 120 | 0 | **0** | **0** | **0** |

即**链路层无丢字节**，`target` 回读与最后一条 SET 目标一致（固件确实接受了 gripper 目标）。

> ⚠️ **实测过程本身给出一条纪律**：早期版本的诊断脚本报"缺 221 条 / 全帧无 ACK"，
> 看起来完美复现了缺陷 —— 但那**全是脚本自身的度量缺陷**（缓冲里有货却因串口暂无新字节
> 而返回 `None`，令 `drain()` 提前收工）。教训：**一个"高丢帧率"的读数未必是缺陷**，
> 能一票否决它的只有**独立的计数器**（`STATS`）。工具侧修正见
> `MeArm-3D/robot-package/mearm-v1/tools/probe_gripper_link.py`。

### 板子复位后的「吞命令」窗口（上位机必读）

打开串口会使 Uno 复位（DTR 断言）。复位后存在**两个**吞字节窗口，各消耗一条指令：
```
try0: ''                                  ← 窗口①（横幅还没出来）
...
```
**这不是固件缺陷**：主循环是非阻塞的（`cmd_poll()` 每轮都跑），
而是复位后 UART 的稳定窗口（横幅由 `uart_puts` 在 `sei()` 之后立即打出，
此时 RX 路径尚未完全就绪）。

⚠️ 另外 **DTR 边沿并非每次都触发复位**（连续开关端口时尤其如此），
所以"是否看到横幅"**不能**作为就绪判据 —— 唯一可靠的定义是**能应答**。
上位机应做**重试探针**（反复发一条轻量命令直到收到回执），而不是固定等待。
