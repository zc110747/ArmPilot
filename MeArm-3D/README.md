...
**实测效果**：大臂 / 小臂由亮蓝（`#4d8fe8` / `#6aa0ff`）变为**近黑 + 几枚螺栓亮点**，
确实更像真机（实物就是黑色工程塑料）；但**细节在深色光照下偏暗** ——
若后续要更"像"，该动的是**光照 / 材质**，而不是纹理本身。

**下一步（按优先级）**：

- **A. 补拍其余 7 块板**（`base_link` / `column_link` / `column_link_side` / `upper_arm_brace` /
  `forearm_brace` / `tool_link` / `jaw_link`）：需要额外姿态 / 机位，是"更像真机"的主要增量；
- **B. 修四角**：当前框比板大约 30%（四边亮比例 18~62%）。需要一条**在定义上无歧义**的
  收紧方法，不能回到"调目标函数凑"（D59）；
- **C. 光照 / 材质调优**：让近黑纹理的细节可见（成本最低，随时可做）。

> ⚠️ 上一轮给的选项"把线缆拨离板面"**已作废**：线缆本就在板背面，拨它没有意义（D61 四）。
> ⚠️ 上一轮的 **A（前端渲染接入）已完成**，故从待办中移除；"照片纹理值不值"现在有了答案：
> **形体更像了，但信息量很低**（可见内容主要就是那几枚螺栓）—— 是否继续投入取决于要不要走 A。
