...
4. **目标板四角 = 窄带 PCA**（`plates_corners.py`）—— 模型投影已放弃（D60）。
   板本身是一条**直带** ⇒ 沿长轴开一条窄带（半宽 ~35px），带内暗像素 PCA 取主轴极值四角；
   带外的一切（螺栓 / 立柱 / 平行杆 / 线缆）被**结构性排除** ——
   这是绕开"整条臂是同一连通域"（第 3 条）的正解。
   实测大臂 **241×65px**（长宽比 3.69 vs 真值 3.36）、小臂 **200×72px**（2.76 vs 3.78）。
   ⚠️ 中心线端点仍来自**目视读数**（±15px）⇒ 精度有限，**一律目视复核**。
   已走过的两条死路（详见 D60）：
   ① `project_plates.py` 的三项相乘目标函数**欠定** —— 只优化"框内暗色"时 `s=2.5`
   会把三块板挤进左下底座暗区拿到 97.4% 的**假性满分**（而 `oy=770` 已在画面外）；
   ② 整臂剪影 IoU 配准（`fit_arm.py`）上限只有 **30.2%** —— 模型几何与 3D 打印实件有
   **系统性差异**，且物理臂实际角度与下发命令差 ~15°（MG90S 开环无回读）。
...
**纹理已出 2 块**（`make_texture.py --corners`，四角再向心收缩 8% 以避边缘）：
...
⚠️ **大臂那条线缆是无解的**：实拍中线缆**悬在板前面**，只要框圈住板就必然含它 ——
向心收缩只能去**边缘**、去不掉**中间**。**软件层面没有办法**（D60 四）。

**还没做的 7 块**（`base_link` / `column_link` / `column_link_side` / `upper_arm_brace` /
`forearm_brace` / `tool_link` / `jaw_link`）：在 `shoulder=45` 位**被遮挡或不在视野**，
需要额外姿态 / 机位逐一补拍。
...
**下一步（先定物理条件，再谈算法）**：
- **A. 改善物理条件后重拍（推荐）**：把**线缆拨离板面**（或改走线），让目标板"干净"；
  再按需补拍其余 7 块板。四角仍走窄带 PCA + 目视复核；
- **B. 接受现状**（小臂可用、大臂带一条线缆），直接做**前端渲染接入**
  （按面材质数组 + 孔洞掩膜）验证渲染路径 —— 但只有 2/9 块纹理，效果有限。
