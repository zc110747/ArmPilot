...
sys.path.insert(0, str(_REPO_ROOT / "core" / "python"))
...
#: 本工具**只**服务 MeArm-V1（显式写 id，不读选择器 `default`）。
MEARM_V1 = "mearm-v1"

#: 真值路径来自该包 manifest 的**声明**（Phase 2 之前这里写死 `config/robot.yaml`，
#: 真值随包搬走后那种写法会**静默读到不存在的位置**）。
try:
    YAML = declared_path(MEARM_V1, "model.config")
