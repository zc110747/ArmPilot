...
- ★★ **改真值（robot.yaml 的 kinematics/actuators/物理量）的连锁清单 —— 照抄这个顺序**（D80 实践）：
  ① 重生成产物 `gen_model.py` + `gen_urdf.py` → ② `freeze_baseline.py --update`
  （★ **路径键必须逐位不变**，变了说明路径动了）→ ③ `gen_mearm_v1_baseline.py` +
  `run_sim2sim.py --freeze` → ④ 修**写死旧值的断言**（前端/Go 都有；★ 能引用
  `limits.min/max` 就别写死端点，否则每次修订都制造假红）→ ⑤ 同步文档（**文档↔配置一致性在 pytest 里盯**）。
  ⚠️ 改方向类标定（如 `reverse`）时，**检查每处"越界钳位"断言的钳位端是否翻转** ——
  D80 就踩到：θ 越大舵机角越小 ⇒ 原本"钳到上限"的用例会变成钳到下限。
- ★ **夹爪（S6）真机方向：S6=40 张开 / S6=130 闭合**（2026-09-15 用户实测，ADR **D80**）。
  `servo_6 = -θ + 140`（offset 140 / reverse true），`gripper.limit = 10..100`。
  ⚠️ **不能只翻 `reverse`**：`homePose.gripper=50` 必须反算出 S6=90（固件开机位），
  而关节限位 `0..90` 本身是照旧（错）方向定的 ⇒ 限位必须一并由舵机硬限位反算。
  ⚠️ **遗留**：真机方向仅来自用户口述，**无相机判据**（`verify_pose.py` 无爪开合反解）。
- ★★ **诊断工具会自己误报**（2026-09-15）：脚本报"缺 221 条/全帧无 ACK"，
  固件 `STATS` 实测却是 **240/240、rx_drop=0** ⇒ 全是工具度量缺陷。
  **铁律：否定"链路丢字节"只能靠独立计数器（`rx_drop`），不能用收发对账。**
  细节（4 个陷阱 + 复位吞命令双窗口）见 `playbook.md` **§12**。

## 二、验收七件套

**完整命令见 `playbook.md` §11**（前端三件：tsc / vitest / vite build / ui-smoke；
Python 六件：pytest / run_sim2sim --all / 两条 `--check` / `validate --all`）。
实测基线（2026-09-15，D80 后）：`pytest -q` **198 passed** · `test_baseline_frozen.py` **11 passed** ·
`validate --all` 通过 · 前端 vitest **401 passed** · `go test ./...` 全绿。
固件（AVR，`MeArm-Device`）：FLASH **12848 B** / RAM **844 B**，零警告。
...
| **§12 固件链路诊断：独立计数器 vs 工具自误报 · 复位吞命令双窗口 · `STATS` 命令** | `playbook.md` **§12** |
