...
两台的 FK 残差量级差 10 个数量级，而原因完全不同：

* `mearm-v1` —— `robot.yaml` 与 `fkref.py` 都是我们写的、同源 ⇒ `~1e-13 mm`
* `so-arm101` —— 官方 URDF 的 `<origin rpy>` 只保留 **6 位有效数字**
  （`1.5708 ≠ π/2`、`3.14159 ≠ π`），而官方 MJCF 用四元数、归一化后恰好 90°
  ⇒ 两份官方文件**自身**就有 µm 级差异 ⇒ `~3.5e-3 mm`
...
FK_TOL_MM: dict[str, tuple[float, str]] = {
    ...
    "so-arm101": (
        5e-2,
        "官方 URDF 的 <origin rpy> 截断到 6 位有效数字（1.5708≠π/2、3.14159≠π），"
        "官方 MJCF 用四元数归一化后恰好 90° ⇒ 两份**官方文件自身**差 µm 级；"
        "实测 ~3.5e-3 mm，5e-2 留一个数量级余量（不放宽到能盖住真错）",
    ),
}
