...
joystick:             # 网页双摇杆 -> 舵机映射（方向反了就把对应 invert 改回 false）
  ...
  invert_rx: true
  invert_ry: true
  deadband_deg: 10    # 动作死区(度)：偏移<=该值不下发；四轴全居中时整帧跳过(串口零流量)
log_level: "info"
```
...
拖拽网页摇杆：左右 → 底座(S9)旋转，前后 → 左舵(S8)俯仰。**动作死区 10°**（视觉倾角，
满偏约 31.5°）：偏移 ≤10° 的轴不下发；**四轴全部 ≤10° 时整帧 JOY 都不发**（串口零流量）；
一旦超过 10° 立即进入有效步进区间（跨过固件 raw 200/800 阈值），偏移越大步进越快
（2~10°/次），松手回中即停。死区可用 `joystick.deadband_deg` 调整。

右侧"舵机角度"面板常显 S6~S9 实时角度：服务端每 500ms 轮询一次 STATUS（仅在有
浏览器连接且串口在线时），并从所有含角度的应答（STATUS / SET / JOY）合并更新。
