 ...
 */
import { defineRobot, type RobotDefinition } from '@robot/definition/RobotDefinition';
...
import type { JointState, Pose, Vec3 } from '@robot/model/Pose';
import { forwardKinematics } from '@robot/kinematics/fk';
import { solveIk, type IkPreference } from './ik';
import { type IKResult } from '@robot/kinematics/IKResult';
import { fromMeArmIkResult } from './fromMeArmIkResult';
import type {
  InverseOptions,
  KinematicsCapability,
  KinematicsEngine,
  RobotPoses,
} from '@robot/kinematics/KinematicsEngine';
...
/**
 * ★ **包引擎模块的统一出口** —— 前端 `RobotRegistry` 用
 * `import.meta.glob('robot-package/&lt;id&gt;/kinematics/engine.ts')` 自动发现本文件后，
 * **只认这一个名字**（见 `frontend/src/robot/registry/RobotRegistry.ts`）。
 *
 * 为什么必须有一个统一名字：Registry 是**唯一**允许按机器人分派的地方，
 * 而"每个包用不同的工厂名"会把分派重新散回 Registry（一张 if/else 表）。
 * 有了它，Registry 只需要知道"包在哪、引擎怎么建"这两件事。
 *
 * 刻意**强制接收 `definition`**（而不是允许省略、由引擎自己去 load）：
 * 于是"引擎算的是哪一个定义"永远由调用方决定，不会出现
 * "Registry 拿了 A 的定义、引擎内部又 load 了 B"这种静默错配。
 */
export function createKinematicsEngine(definition: RobotDefinition): KinematicsEngine {
  return new MeArmKinematics(definition);
}
