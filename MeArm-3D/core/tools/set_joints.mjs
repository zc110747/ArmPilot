 ...
 *
 * 用法
 *   node core/tools/set_joints.mjs --status                     # 只读，不动臂
 *   node core/tools/set_joints.mjs --home                       # 回 HOME 位（四关节一起）
 *   node core/tools/set_joints.mjs shoulder=20 elbow=125        # 只改指定关节
 *   node core/tools/set_joints.mjs 0 20 125 50                  # 位置式: base shoulder elbow gripper
 *   node core/tools/set_joints.mjs --settle 1200 shoulder=10    # 自定义到位等待
 *
 * 为什么需要它
 ...
 * ⚠️ 安全设计（五条，都不可省）
 *
 *   1. **限位来自 hello，不硬编码**。robot-package/mearm-v1/model/robot.yaml 是唯一真值，本脚本不抄一份。
...
function usage(code = 0) {
  console.log(`用法:
  node core/tools/set_joints.mjs --status                    只读当前关节状态（不动臂）
  node core/tools/set_joints.mjs --home                      回 HOME 位（homePose，来自 hello）
  node core/tools/set_joints.mjs shoulder=20 elbow=125       只改指定关节，其余保持不变
  node core/tools/set_joints.mjs 0 20 125 50                 位置式 base shoulder elbow gripper
选项:
...
ws.addEventListener('message', (ev) => {
  ...
  if (m.type === 'hello') {
    ...
    console.log(`限位（来自 ${m.model?.source ?? 'robot-package/mearm-v1/model/robot.yaml'}，本脚本不硬编码）:`);
  ...
  if (m.type === 'joint_state') {
    ...
    if (cfg.verify) {
      ...
      if (bad.length) {
        ...
        console.error('  ↑ 限位真值在 robot-package/mearm-v1/model/robot.yaml。不要为了"让它动"而放宽校验。');
        finish(2);
        return;
      }
    }
    ...
    setTimeout(() => {
      finish(0, `\n✓ 指令已下发（等待 ${cfg.settle}ms）。真机无回读，此处不代表已物理到位。\n` +
        `  下一步：抓帧看实际效果 —— python robot-package/mearm-v1/tools/capture_texture.py --plate <name>`);
    }, cfg.settle);
    return;
  }
