...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    ap.add_argument("-o", "--out", default=str(Path(__file__).resolve().parent / "mearm.xml"),
                    help="输出路径（默认 simulation/mujoco/mearm.xml）")
    ap.add_argument("--stdout", action="store_true", help="只打印，不写文件")
    ap.add_argument("--robot", default=None, help="robot.yaml 路径（默认 config/robot.yaml）")
    ap.add_argument("--physics", default=None, help="physics.yaml 路径（默认 config/physics.yaml）")
