 ...
 *
 * ⚠️ 本文件**不含任何限位/角度常量**（项目铁律：真值只有 `robot-package/mearm-v1/model/robot.yaml` 一份）。
 *    位姿一律由调用方以 `--joints` 传入。想拿"限位钳位后的零位"，请自己从
 *    `robot-package/mearm-v1/model/robot.yaml` 读，例如：
 *
 *      python -c "import yaml,json;m=yaml.safe_load(open('robot-package/mearm-v1/model/robot.yaml'));\
 *      print(json.dumps({j['id']:min(max(0,j['limits']['min']),j['limits']['max'])\
 *      for j in m['robot']['joints']}))"
 *
 * 用法
 * ----
 *   node core/tools/park_sim_pose.mjs <wsUrl> --joints '{"base":0,"shoulder":-6.0,...}'
 *   node core/tools/park_sim_pose.mjs <wsUrl> --joints @file.json
 *
 * 可选：
...
function usage(message) {
  ...
  console.error(
    'usage: node core/tools/park_sim_pose.mjs <wsUrl> --joints \'{"base":..,"shoulder":..,"elbow":..,"gripper":..}\'',
  );
  process.exit(2);
}
