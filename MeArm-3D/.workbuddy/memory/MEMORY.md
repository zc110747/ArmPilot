...
- **★★ 动关节改变不了 `px/mm`**（2026-09-13 实测定论，本轮最重要的结论）：
  ...
  - ⇒ **唯一手段是物理靠近相机**。⚠️ 但**具体 `s` 值不可信**：旧值 `3.80` 实为搜索区间
    **下界被截断**的结果，放宽区间后变成 2.58（两者互相矛盾，D59 ①）。
    **只能按"越近越大"定性地走**，不要引用某个具体 `s`。
- **★★ 「白卡隔离出单块板」不成立**（推翻本条旧说法）：
  ...
  **白卡挡得住背景，挡不住结构性连通。**
- **★ 目标板四角 = 窄带 PCA**（`plates_corners.py`，D60）——**模型投影已放弃**。
  整臂剪影 IoU 配准上限只有 **30.2%**：模型几何与 3D 打印实件有**系统性差异**（叠加图显示
  模型的臂整体比实拍"高"，调角度补不上），且物理臂实际角度与下发命令差 **~15°**
  （MG90S 开环无回读的必然）。⚠️ 投影时**必须排除 `table_top`**（约 1m 见方，
  会撑成全屏矩形使 IoU 退化成"实际面积占比"）。
  窄带 PCA：板本身是一条**直带** ⇒ 沿长轴开窄带（半宽 ~35px），**带内**暗像素 PCA 取
  主轴极值四角；带外一切（螺栓/立柱/平行杆/线缆）**结构性排除** ——
  这是绕开"整条臂是同一连通域"（本条下面那段）的正解。
  实测大臂 241×65px（长宽比 3.69 / 真值 3.36）、小臂 200×72px（2.76 / 3.78）。
  ⚠️ 中心线端点仍是**目视读数**（±15px）⇒ 最终纹理**一律目视复核**。
  （已走过的死路：`project_plates.py` 三项相乘目标函数**欠定** —— 只优化"框内暗色"时
  `s=2.5` 把三块板挤进左下底座暗区拿到 **97.4% 假性满分**，而 `oy=770` 已在画面外。）
...
- **★ 纹理质量的上限由物理条件决定（D60）**：必须同时满足 ① 目标板**完整入画**
  ② **不被线缆横穿** ③ 目标面**正对相机**。线缆**悬在板前**时**软件无解**
  （四角向心内缩只能去边缘、去不掉中间）。其余 7 块板（`base_link` / `column_link` /
  `column_link_side` / 两 brace / `tool_link` / `jaw_link`）在单个姿态下**被遮挡或不在视野**，
  需要额外姿态/机位逐一补拍。已有纹理：`forearm_link` ✅ 干净 · `upper_arm_link` ⚠️ 含一条橙线缆。
...
- **★★ 结论：自动定位目标板在本机场景走不通（D59）** —— 不要再投入"调目标函数让它对上"。
  ...
  ⇒ **出纹理只认「窄带 PCA 定位 + 目视复核」**（`make_texture.py --corners`），
  自动投影降级为"粗定位参考"。
